...
// GET /departments -- returns all department names
...
// GET /tickets/details -- note return all tickets with joined user and department names
...
// GET /tickets/:id -- returns tickets by id
...
app.get('/ticket-notes/:ticketId', async (req,res) => {
    try{
        ...
        console.error('Error getting ticket notes for ticket:', error);
        res.status(500).json({error:'Failed to get ticket notes.'});
    }
});
...
app.post('/users', (req,res) => {
    ...
    // password rule 1:must be 8 characters
    if(password.length < 8) {
        return res.status(400).json({error:"Password must be at least eight characters long."});
    }
    
    // password rule 2:must be contain a special character !@#$% 
    const specialChar = /[!@#$%]/
...
// POST /login -- validates Credential;s and returns user info with role
